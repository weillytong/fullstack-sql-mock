...
const insertMockData = function() {
  ...
  for (let i = 0; i < productsArr.length; i++) {
    ...
    let queryString = `INSERT INTO products (item, min_cost, curr_bid, ends_in, image) VALUES ('${product.item}', ${product.min_cost}, ${product.curr_bid}, ${product.ends_in}, '${product.image}')`
    db.query(queryString, (err, results) => {
      if (err) {
        console.log(err);
      } else {
        console.log('successful seed insert')
      }
    })
  }
};
